...
> **Статус:** ✅ Реализовано на LangGraph + Web Search
...
Этот документ описывает **текущую реализацию** сервиса **brama** на основе **LangGraph** с интеграцией **Perplexity Web Search**.

### Текущая архитектура (v4.0)

Система построена на **LangGraph workflow** с 5 последовательными этапами обработки запроса:

1. **QueryAgent** - анализирует запрос через LangGraph (5 этапов)
2. **DashboardService** - генерирует финальный dashboard с графиками
3. **PerplexityClient** - выполняет веб-поиск для дополнения данных
4. **Shared modules** - переиспользуемые клиенты (LLM, Neo4j, Perplexity, Logger)
...
<details>
<summary><b>v3.0 → v4.0: Добавление Perplexity Web Search и улучшение выбора ячеек</b></summary>

#### Что изменилось:

**v3.0:**
- 4 этапа LangGraph workflow
- Выбор одного представления
- Только данные из Neo4j

**v4.0:**
- ✅ **5 этапов LangGraph workflow** (добавлен perplexityWebSearch)
- ✅ **Множественный выбор представлений** - LLM может выбрать несколько таблиц
- ✅ **Поиск похожих ячеек** - внутри схемы таблицы ищутся аналогичные колонки/строки
- ✅ **Интеграция Perplexity API** - веб-поиск для дополнения данных
- ✅ **Умный выбор режима поиска** - LLM анализирует качество данных и выбирает 'fast' или 'deep'
- ✅ **Отдельная очередь для веб-поиска** - результаты доставляются через telegram-web-search queue
- ✅ **Graceful degradation** - система работает даже если веб-поиск недоступен

#### Почему изменили:

✅ **Полнота данных:** Веб-поиск дополняет данные из Neo4j актуальной информацией
✅ **Интеллектуальный поиск:** LLM сам решает когда нужен быстрый, а когда глубокий поиск
✅ **Улучшенная точность:** Множественный выбор представлений и поиск похожих ячеек повышает релевантность
✅ **Независимость компонентов:** Веб-поиск в отдельной очереди не блокирует основной dashboard
✅ **UX:** Пользователь получает dashboard сразу, а веб-поиск приходит отдельным сообщением
...
sequenceDiagram
    ...
    Backend->>Backend: 🔴 POST /api/clarify<br/>{taskId: uid, question}
    Backend->>Redis: 🔴 Добавляет задачу clarify в Queue

    Note over Redis,Worker: Worker Pool (concurrency=2)

    Worker->>Redis: Извлекает Job из очереди
    Worker->>DashGen: 🔴 getClarifications(question)
    DashGen->>QAgent: 🔴 clarifyQuery(question)

    rect rgb(255, 200, 200)
        Note over QAgent,Neo4j: 🔴 НОВЫЙ ЭТАП: УТОЧНЕНИЕ ЗАПРОСА

        QAgent->>Neo4j: 🔴 Получить список СТАТФОРМ + разделы
        Neo4j-->>QAgent: [Статформы с описаниями...]
        QAgent->>QAgent: 🔴 LLM генерирует 4 варианта<br/>уточнения запроса
    end

    QAgent-->>DashGen: 🔴 ClarificationResult {suggestions: [...]}
    DashGen-->>Worker: 🔴 ClarificationResult
    Worker->>Backend: 🔴 POST /api/queries/callbacks/:uid<br/>{status: 'clarification_needed', suggestions}

    Backend->>DB: 🔴 Обновляет Query (suggestions)
    Backend->>Redis: 🔴 Добавляет в telegram-notifications

    Redis-->>TgBot: 🔴 {chatId, suggestions}
    TgBot->>User: 🔴 Выберите вариант:<br/>1️⃣ Вариант 1<br/>2️⃣ Вариант 2<br/>3️⃣ Вариант 3<br/>4️⃣ Вариант 4

    User->>TgBot: 🔴 Нажимает кнопку варианта
    TgBot->>Backend: 🔴 POST /api/queries/:uid/select<br/>{selectedVariant: 2}
    ...
    rect rgb(240, 248, 255)
        Note over QAgent,Neo4j: ✅ LANGGRAPH WORKFLOW (5 этапов)
        ...
        QAgent->>Neo4j: Этап 3: Получить представления раздела + схему таблиц
        Neo4j-->>QAgent: [Таблица 1, Таблица 2, ...] + схемы
        QAgent->>QAgent: ✅ LLM выбирает ячейку + похожие<br/>→ viewId, colIndex, rowIndex

        Note over QAgent: ✅ НОВЫЙ Этап 4: Веб-поиск (Perplexity)
        QAgent->>QAgent: LLM анализирует данные<br/>Выбирает режим: 'fast' или 'deep'
        QAgent->>QAgent: Формирует поисковый запрос
        QAgent->>QAgent: Вызов Perplexity API
        Note over QAgent: Если поиск не удался → graceful degradation

        QAgent->>DashSvc: Этап 5: generateDashboard({<br/>  viewId, colIndex, rowIndex,<br/>  metadata<br/>})
    ...
    QAgent-->>DashGen: DashboardData + WebSearchResult
    DashGen-->>Worker: DashboardData + WebSearchResult

    Worker->>Backend: POST /api/queries/callbacks/:uid<br/>{status: 'completed', result: {dashboard, webSearchResult}}

    Backend->>DB: Обновляет Query<br/>(result, status='completed')
    Backend->>Redis: ✅ Добавляет в telegram-notifications (dashboard)
    Backend->>Redis: ✅ НОВОЕ: Добавляет в telegram-web-search (webSearchResult)

    Redis-->>TgBot: Уведомление dashboard<br/>{chatId, uid, dashboardUrl, chartDescription, yearlyData}
    Redis-->>TgBot: ✅ НОВОЕ: Уведомление веб-поиск<br/>{chatId, uid, searchMode, content, sources, query}

    TgBot->>User: ✅ Дашборд готов!<br/>📊 [Описание + данные]<br/>🔗 https://dashboard.url/uid
    TgBot->>User: ✅ НОВОЕ: 🔍 Веб-поиск<br/>[Режим: fast/deep]<br/>[Контент]<br/>🔗 [Источники]
...
**PerplexityClient** - клиент для работы с Perplexity API
```typescript
fastSearch(query: string): Promise<PerplexityResponse>    // Режим 'sonar' - быстрый поиск
deepResearch(query: string): Promise<PerplexityResponse>  // Режим 'sonar-deep-research' - глубокий анализ
```


---

## LangGraph Workflow (5 этапов)
...
interface QueryState {
  ...
  webSearch?: WebSearchResult;        // ✅ НОВОЕ: Результат этапа 4 (веб-поиск)
  dashboardData?: DashboardData;      // Результат этапа 5
  error?: string;
}

interface WebSearchResult {
  searchMode: 'fast' | 'deep';        // Режим поиска (выбирается LLM)
  content: string;                     // Текст результата
  sources?: string[];                  // Источники
  query: string;                       // Запрос для поиска
}
```

> **Файл:** `brama/src/query-agent/graph/state.ts`
...
**✅ УЛУЧШЕНИЯ в v4.0:**
- **Множественный выбор представлений**: LLM может выбрать несколько таблиц (не только одну)
- **Поиск похожих ячеек**: Внутри схемы таблицы автоматически находятся аналогичные колонки/строки

**Процесс:**
1. Получить список представлений (view) для раздела
2. Для каждого представления получить схему таблицы (заголовки колонок и строк)
3. Создать промпт со структурой всех доступных таблиц
4. LLM выбирает представление(я) и координаты ячейки с учетом похожих вариантов
5. Сохранить `viewId`, `colIndex`, `rowIndex`, `metadata` в State

**Пример:**
```
Запрос: "Сколько школьников в Москве?"
→ Схема таблиц (множественный выбор):
  Таблица 1 (городские школы):
    Колонки: ["№", "Наименование", "Всего", "1 класс", "2 класс", ...]
    Строки: ["Москва", "Санкт-Петербург", ...]
  Таблица 2 (сельские школы):
    Колонки: ["№", "Наименование", "Всего", "1 класс", "2 класс", ...]

→ LLM анализирует и выбирает:
  - viewId: 456 (Таблица 1 - городские)
  - colIndex: 2 ("Всего") + похожие: [3 "1 класс", 4 "2 класс", ...]
  - rowIndex: 1 ("Москва")
...
### Этап 4: Веб-поиск (perplexityWebSearchNode) ✅ НОВЫЙ

**Цель:** Дополнить данные из Neo4j актуальной информацией из интернета

**Процесс:**
1. Анализ результатов этапа 3 (viewSelection)
2. LLM оценивает качество данных и формирует поисковый запрос
3. LLM выбирает режим поиска:
   - `'fast'` (sonar) - если данные хорошего качества, нужно только дополнение
   - `'deep'` (sonar-deep-research) - если данные неполные или нужен глубокий анализ
4. Выполнение запроса к Perplexity API
5. Сохранить `webSearch` в State (или пропустить если поиск не удался)

**Пример:**
```
Запрос: "Общая численность школьников в России 2025 год"
→ viewSelection содержит данные только до 2024 года

→ LLM анализирует:
  - Данные есть за 2021-2024
  - Пользователь спрашивает про 2025 (данных нет)
  - Нужен веб-поиск для актуальных данных

→ LLM выбирает режим: 'fast'
→ Формирует запрос: "общая численность школьников в России 2025 год"

→ Perplexity API возвращает:
  {
    searchMode: 'fast',
    query: 'общая численность школьников в России 2025 год',
    content: 'По данным Минпросвещения на начало 2024/2025 учебного года...',
    sources: ['https://edu.gov.ru/...', 'https://rg.ru/...']
  }

→ State.webSearch = { searchMode: 'fast', content: '...', sources: [...], query: '...' }
```

**Graceful Degradation:**
- Если Perplexity API недоступен или возвращает ошибку → `webSearch = undefined`
- Workflow продолжается без веб-поиска
- Dashboard генерируется только на основе Neo4j данных

---

### Этап 5: Генерация dashboard (generateDashboardNode)
...
```mermaid
graph TD
    Start([🚀 START]) --> S0{{"🔴 НОВЫЙ Этап 0<br/>clarifyQuery"}}

    S0 --> State0["📦 State<br/>query: string<br/>🔴 clarification: {<br/>  suggestions: [4 варианта]<br/>}"]

    State0 --> UserChoice([🔴 Возврат пользователю<br/>для выбора варианта])

    UserChoice --> S1{{"🔹 Этап 1<br/>selectStatform"}}

    S1 --> State1["📦 State<br/>query: string (уточненный)<br/>statformSelection: {<br/>  statformIds: [852]<br/>}"]
...
graph TD;
	...
	clarifyQuery(🔴 clarifyQuery - НОВЫЙ):::clarifyQuery
	...
	clarifyQuery -.->|"🔴 После выбора пользователя"| selectStatform;
...
### Формат callback в Backend ✅ ОБНОВЛЕНО в v4.0
...
# ✅ НОВОЕ: Perplexity API для веб-поиска
PERPLEXITY_API_KEY=pplx-xxxxxxxx         # API ключ Perplexity (обязательно для веб-поиска)
...
**Зачем нужен Perplexity:**
- Веб-поиск с актуальной информацией из интернета
- Дополнение данных из Neo4j свежими данными
- Два режима: `sonar` (быстрый) и `sonar-deep-research` (глубокий анализ)
- LLM автоматически выбирает режим в зависимости от качества данных

**Стоимость:**
- `sonar` (fast mode): ~$1 за 1000 запросов
- `sonar-deep-research` (deep mode): ~$5 за 1000 запросов

#### 3. LangSmith API Key (опционально)
...
- Детальная трассировка всех 5 этапов LangGraph
...
- [x] ✅ **5 последовательных этапов обработки** (добавлен веб-поиск)
- [x] ✅ **Множественный выбор представлений** в этапе 3
- [x] ✅ **Поиск похожих ячеек** внутри схемы таблицы
- [x] ✅ **Perplexity Web Search** с умным выбором режима (fast/deep)
- [x] ✅ **Отдельная очередь telegram-web-search** для доставки результатов
- [x] ✅ **Graceful degradation** при недоступности веб-поиска
- [x] DashboardService генерирует финальный dashboard
- [x] Shared modules (LLMClient, Neo4jClient, PerplexityClient, Logger)
- [x] Поддержка LangSmith трассировки
- [x] Поддержка параллельных вызовов (concurrency=2)
- [x] Все ошибки на русском языке
- [x] Backend получает корректный JSON через callback (с опциональным webSearchResult)
- [x] Timeout 60 секунд
